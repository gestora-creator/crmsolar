...
export function ClienteTimeline({ clienteId }: Props) {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useTimelineByCliente(clienteId)
    ...
    <div className="space-y-4">
      ...
      {eventos.length === 0 ? (
        <EmptyState
          ...
          description="Os eventos de interação com este cliente aparecerão aqui conforme forem registrados."
        />
      ) : (
          ...
          {Object.entries(grouped).map(([dateKey, dayEvents]) => (
            <div key={dateKey}>
              ...
              <div className="space-y-2">
                {dayEvents.map((ev) => (
                  <TimelineEventCard key={ev.id} event={ev} showRelacionamento />
                ))}
              </div>
            </div>
          ))}
